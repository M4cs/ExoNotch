...
@implementation EXNPrefsListController
- (instancetype)init {
    self = [super init];
...
- (void)addThemeSpecifiers {
    ...
    HBPreferences *prefs = [[HBPreferences alloc] initWithIdentifier:EXNPrefsIdentifier];
    if ([prefs objectForKey:@"Theme"]) {
        themeName = [prefs objectForKey:@"Theme"];
    }

    EXNTheme *theme = [EXNTheme themeWithDirectoryName:themeName];
    ...
    for (int i = [settings count] - 1; i >= 0; i--) {
        ...
        [specifier setProperty:EXNPrefsIdentifier forKey:@"defaults"];
        [specifier setProperty:EXNNotification forKey:@"PostNotification"];
...
- (void)refresh:(id)sender {
    CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), (CFStringRef)EXNRefreshNotification, nil, nil, true);
}

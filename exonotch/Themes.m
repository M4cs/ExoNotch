...
@interface EXNAlignedTableViewCell : UITableViewCell
...
@implementation EXNAlignedTableViewCell
...
@implementation EXNThemesListController
...
- (id)initForContentSize:(CGSize)size {
    ...
    if (self) {
        ...
        [_tableView registerClass:[EXNAlignedTableViewCell class] forCellReuseIdentifier:@"cell"];
...
- (void)addThemesFromDirectory:(NSString *)directory {
    ...
    for (NSString *dirName in diskThemes) {
        EXNTheme *theme = [EXNTheme themeWithDirectoryName:dirName];
...
- (void)refreshList {
    self.themes = [[NSMutableArray alloc] initWithCapacity:100];
    [self addThemesFromDirectory: EXNThemesDirectory];
    ...
    HBPreferences *file = [[HBPreferences alloc] initWithIdentifier:EXNPrefsIdentifier];
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    EXNAlignedTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    
    EXNTheme *theme = [self.currentThemes objectAtIndex:indexPath.row];
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    EXNTheme *theme = (EXNTheme*)[self.currentThemes objectAtIndex:indexPath.row];
    selectedTheme = theme.name;

    HBPreferences *file = [[HBPreferences alloc] initWithIdentifier:EXNPrefsIdentifier];
    [file setObject:selectedTheme forKey:@"Theme"];

    EXNPrefsListController *parent = (EXNPrefsListController *)self.parentController;
    [parent setThemeName:selectedTheme];

    CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), (CFStringRef)EXNNotification, nil, nil, true);
}
